...
for label in labels:
    ...
    fig.supylabel('Total number of instances')
    ax1.set_xlabel('Predicted value for toxic')
    ax1.set_ylabel('')
    ax2.set_xlabel('Predicted value for non-toxic')
    ax2.set_ylabel('')
    ax1.set_title('Amount of predictions dependent on value')
    ...
    plt.plot(recall, precision, label='PR Curve', color='navy')
    plt.axhline(y = percentile, color = 'r', linestyle = '--')
    ax.legend(['precision - recall curve', 'percentage of positive class\ninstances in the set'])
    plt.xlabel('Recall')
    ...
    plt.plot(thresholds, true_ones, color='r')
    plt.plot(thresholds, false_ones, color='navy')
    plt.title('Amount of assigned toxic labels on different thresholds')
    plt.xlabel('Threshold value')
    plt.ylabel('Total number of instances above threshold')
    plt.xticks(np.arange(0.1,1.1,0.1))
    plt.legend(['toxic', 'non-toxic'], title='real class')      
    plt.savefig('threshold_plots/threshold_'+label+'.png')
    plt.close()

    # overlapping hist
    sns.histplot(bins=20, binrange=[0,1], data=res[label][~np.isnan(res[label, "Real Value"])], alpha=0.5, palette=['blue', 'red'], x="Prediction", hue="Real Value", log_scale=(False,True), legend=False)
    plt.title('Logarithmic amount of predictions dependent on value')
    plt.ylabel('log( Total number of instances )')
